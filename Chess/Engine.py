...
# Return the type of piece give it's position. Return none if empty
def getType(side, board, pos):
    for piece in board[side]:
        if piece[:2] == pos:
            return piece[2]

# Determine whether the position given is occupied by a piece of the given side 
def isOccupied(side, board, pos):
    return getType(side, board, pos) is None

# Determine whether the position(s) given is empty not
...
# This function returns whether a game has ended or not
def isEnd(side, board, flags):
    for _ in legalMoves(side, board, flags):
        return False
    return True

# This function moves the piece from one coordinate to another while handling the
# capture of enemy, pawn promotion and en-passent
# One thing to note that this function directly modifies global value of the board
# variable from within the function, so pass a copy of the board
...
# This function returns whether a move puts ones own king in check
